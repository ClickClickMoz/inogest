							...
							<div class="card">
								...
								<br>
										...
										<div class="row">
											...
											<div class="card">
												...
												if(isset($_GET['buttonPesquisar']) and $_GET['txtpesquisar']!=''){
													...
													$queryTabela = "SELECT * FROM `pagamentos` where datar = '$data'";
												}else{
                                                   
												$queryTabela = "SELECT * FROM `pagamentos` order by id desc";
												}
													...
													<tbody>
														...
														while($res_1 = mysqli_fetch_array($resultTabela)){
															...
															<tr>
																...
																pages-pagamentos.php?func=deleta&id=<?php
																echo $id;
																?>
																">
										...
										<div class="modal-body">
											...
											<label for="id_produto">Funcionario</label>
											<select class="form-control mr-2" id="category" name="txtfuncionario">
                                                <?php
                                                ...
                                                while($res_2 = mysqli_fetch_array($result)){
                                                        ?>                                             
                                                <option value="<?php echo $res_2['nome']; ?>"><?php echo $res_2['nome']; ?></option> 
                                                        <?php      
                                                    }
                                                }
                                                ?>
											...
											<label for="fornecedor">Valor</label>
												<input type="text" class="form-control mr-2" id="txtmotivo" name="txtvalor" placeholder="Valor" required>
											</div>
...
<?php
  ...
  if(isset($_POST['button'])){
    ...
    $query = "insert into `pagamentos`(`valor`, `funcionario`, `responsavel`,`datar`) VALUES ('$valor', '$funcionario','$responsavel', CURRENT_DATE)";
 
    $result =   mysqli_query($conexao, $query);

    //recuperar ultimo id lancado
 
    $queryId = "SELECT * from `pagamentos` order by id desc limit 1"; 
    ...
    $querymov = "insert into `movimentos`(`tipo`, `movimento`, `valor`, `funcionario`,`datar`,`id_movimento` ) VALUES ('Saida', 'Pagamento','$valor', '$responsavel',CURRENT_DATE,'$id_ultimo')";
    ...
    if($result == ''){
      ...
      echo"<script language='javascript'>window.location='pages-pagamentos.php'; </script>";
    }
...
if(@$_GET['func'] == 'deleta'){
  ...
  $querymov = "DELETE FROM movimentos where movimento = 'Pagamento' and id_movimento = '$id'";
  mysqli_query($conexao,$querymov);
